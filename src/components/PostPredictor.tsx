  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      const engagementScore = comments.reduce((score: number, edge: any) => {
        // Simplified: Use static sentiment score instead of doc.sentiment()
        const sentiment = 0.5; // Neutral score as fallback
      ...
      if (writePrediction) {
        writePrediction({ args: [coinAddress, BigInt(Math.floor(predicted * 100)), postTime || new Date().toISOString()] });
      }
    } catch (err: any) {
